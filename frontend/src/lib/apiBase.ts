 ...
 *
 * `window.__KNODE_API_BASE__` 가 있으면 그걸 쓴다 — Electron `preload`
 ...
 * 실행 전에 `window.__KNODE_API_BASE__` 를 심어 주는 게 보통이지만,
 * 그 타이밍을 모듈 로드 순서에 기대지 않기 위함이다.
 */

declare global {
  interface Window {
    __KNODE_API_BASE__?: string;
  }
}

function runtimeOverride(): string | undefined {
  if (typeof window === 'undefined') return undefined;
  return window.__KNODE_API_BASE__;
}
