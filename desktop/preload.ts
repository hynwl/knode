...
const ARG_PREFIX = '--agentcanvas-api-base=';
...
contextBridge.exposeInMainWorld('__AGENTCANVAS_SECRETS_BRIDGE__', {
  read: () => ipcRenderer.invoke('agentcanvas:secrets:read'),
  write: (payload: unknown) => ipcRenderer.invoke('agentcanvas:secrets:write', payload),
  remove: () => ipcRenderer.invoke('agentcanvas:secrets:remove'),
});
...
contextBridge.exposeInMainWorld('__AGENTCANVAS_ONBOARDING_BRIDGE__', {
  status: () => ipcRenderer.invoke('agentcanvas:onboarding:status'),
  markSeen: () => ipcRenderer.invoke('agentcanvas:onboarding:markSeen'),
  revealWorkspace: () => ipcRenderer.invoke('agentcanvas:onboarding:revealWorkspace'),
});
