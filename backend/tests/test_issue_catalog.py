...
BACKEND_TRIGGERED: dict[str, str] = {
    ...
    "AC-E406": "test_documents.py::test_route_unsupported_returns_ac_e406",
    "AC-E407": "test_documents.py::test_route_too_large_returns_ac_e407",
    "AC-E408": "test_documents.py::test_route_corrupt_returns_ac_e408",
...
FRONTEND_ONLY: dict[str, str] = {
    ...
    "AC-E606": "노드가 지목한 키 슬롯이 브라우저 키 저장소에 없다는 판정 — 백엔드는 "
               "슬롯 목록을 모른다(헤더로 온 것만 안다). 프론트 `validateKeys()` 전용(§12.1).",
    "AC-W606": "필요한 키가 브라우저에 등록되지 않았다는 사전 경고. 서버 `.env` 폴백이 "
               "있을 수 있어 백엔드는 이걸 경고로 낼 수 없다 — 백엔드는 실제 실행에서 "
               "AC-E601/AC-E602 로 판정한다. 프론트 `validateKeys()` 전용(§12.1).",
